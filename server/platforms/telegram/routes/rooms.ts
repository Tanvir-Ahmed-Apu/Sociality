...
import { announceRoom, getRoomPeers, getMessagesFromPeer } from '../utils/federation';
...
router.post('/', async (req, res) => {
  try {
    const { name, userId, telegramChatId, telegramUsername } = req.body;
    ...
    // Create Telegram binding if telegramChatId is provided
    if (telegramChatId && telegramUsername) {
      ...
      await binding.save();
      console.log(`Created Telegram binding for room ${roomId} and chat ${telegramChatId}`);
    }
...
router.post('/:roomId/messages', async (req, res) => {
  try {
    const { roomId } = req.params;
    const { text, userId, displayName, platform = 'telegram' } = req.body;
