...
import TelegramBotClient from './utils/TelegramBotClient';
...
import { relayMessageToTelegram, loadAndValidateTelegramBindings, validateTelegramBinding } from './utils/telegramRelay';
...
const bot = new TelegramBotClient(BOT_TOKEN as string);
bot.startPolling();
...
/create "My Cross Platform Room"
  ...
  // Handle /join command
  bot.onText(/\/join (.+)/, async (msg: any, match: RegExpExecArray | null) => {
    const chatId = msg.chat.id;
    if (!match) return;
  ...
  bot.onText(/\/create (.+)/, async (msg: any, match: RegExpExecArray | null) => {
    const chatId = msg.chat.id;
    if (!match) return;
  ...
  bot.on('message', async (msg) => {
    ...
    try {
      ...
      let profilePic = '';
      try {
        const userProfilePhotos = await bot.getUserProfilePhotos(msg.from.id, { limit: 1 });
        if (userProfilePhotos.photos && userProfilePhotos.photos.length > 0) {
          const photo = userProfilePhotos.photos[0][0]; // Get the smallest size
          const fileLink = await bot.getFileLink(photo.file_id);
          profilePic = fileLink;
        }
      } catch (photoError: any) {
        console.log(`Could not get profile photo for user ${username}:`, photoError.message);
      }
