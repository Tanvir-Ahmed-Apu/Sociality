...
// Import utilities
import { registerPlatform, announceRoom, getRoomPeers, getMessagesFromPeer, relayMessageToFederation } from './utils/federation';
import RetryManager from './utils/RetryManager';
import { relayMessageToDiscord, loadAndValidateDiscordBindings, validateDiscordBinding } from './utils/discordRelay';
...
app.get('/api/messages/:roomId', async (req, res) => {
  try {
    ...
    if (since) {
      query.sentAt = { $gt: new Date(since as string) };
    }
    ...
    try {
      ...
      const federatedMessage = {
        ...
        from: {
          ...
          username: username,
          platform: 'discord',
          profilePic: profilePic,                // Add profile picture URL
          avatar: profilePic                     // Keep avatar for backward compatibility
        },
