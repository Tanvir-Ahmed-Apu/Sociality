...
const CommentHeader = ({ reply, username, timeAgo, currentUser, onOpenDeleteAlert, showNewBadge }: CommentHeaderProps) => {
  // Theme-aware colors
  const textColor = useColorModeValue("gray.800", "white");
  const menuBgColor = useColorModeValue("white", "#101010");
  const menuBorderColor = useColorModeValue("gray.200", "gray.700");
  const menuHoverBgColor = useColorModeValue("red.50", "rgba(229, 62, 62, 0.1)");
  const menuTextColor = useColorModeValue("red.500", "red.400");
  const iconHoverColor = useColorModeValue("brand.primary.600", "white");
