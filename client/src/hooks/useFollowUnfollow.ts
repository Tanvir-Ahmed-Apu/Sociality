...
const useFollowUnfollow = (user: User, onFollowToggle: ((user: User) => void) | null = null) => {
	...
	const [following, setFollowing] = useState(
		currentUser?.following?.some(followingUser => {
			...
			const followingId = typeof followingUser === 'string' ? followingUser : (followingUser as any)?._id;
			...
			if (following) {
				...
				if (Array.isArray(user.followers)) {
					user.followers = user.followers.filter(follower => {
						const followerId = typeof follower === 'string' ? follower : (follower as any)?._id;
					...
					const fetchFollowerProfile = async () => {
						try {
							...
							if (res.ok) {
								...
								user.followers?.push(followerToAdd as any);
							}
						} catch (error) {
				...
				if (currentUser) {
					...
					const updatedFollowing = Array.isArray(currentUser.following)
						? [...currentUser.following, followingToAdd as any]
						: [followingToAdd as any];

					const updatedUser = { ...currentUser, following: updatedFollowing as any };
					setCurrentUser(updatedUser);
					localStorage.setItem("user-threads", JSON.stringify(updatedUser));
				}
			}
